...
public class DefaultOrderService implements OrderService {
    ...
    public long placeOrder(CustomerForm customerForm, ShoppingCart cart) {
		...
		// US Mobile Phone Pattern
		private static final String US_MOBILE_PHONE_PATTERN =
				"^((\\+1|1)?( |-)?)?(\\([2-9][0-9]{2}\\)|[2-9][0-9]{2})( |-)?([2-9][0-9]{2}( |-)?[0-9]{4})$";
		private static final Pattern mobilePhonePattern = Pattern.compile(US_MOBILE_PHONE_PATTERN);
	...
	private static final Pattern emailPattern = Pattern.compile(EMAIL_PATTERN);
		...
		private static final Pattern creditCardPattern = Pattern.compile(CREDIT_CARD_PATTERN);

		public static boolean isCreditCard(String input) {
			String sanitized = input.replaceAll("[- ]+", "");

			if (!creditCardPattern.matcher(sanitized).matches()) {
				return false;
			}
			...
			for (int i = sanitized.length() - 1; i >= 0; i--) {
				...
				sum += tmpNum > 9 ? tmpNum % 10 + 1 : tmpNum;
	...
	private void validateCustomer(CustomerForm customerForm) {
		...
		if(customerForm.getAddress() == null ||customerForm.getAddress().length()<4 || customerForm.getAddress().equals("") || customerForm.getAddress().length()>45){
			throw new ApiException.ValidationFailure("Invalid address field");
		}
		if(customerForm.getPhone() == null || !isMobilePhone(customerForm.getPhone())){
			throw new ApiException.ValidationFailure("Invalid phone field");
		}
		if(customerForm.getEmail() == null || !isValidEmail(customerForm.getEmail())){
			throw new ApiException.ValidationFailure("Invalid Email field");
		}
		if(customerForm.getCcNumber()==null || !isCreditCard(customerForm.getCcNumber())){
			throw new ApiException.ValidationFailure("Invalid Credit card field");
		}
		if (customerForm.getCcExpiryMonth()==null && customerForm.getCcExpiryYear() == null && expiryDateIsInvalid(customerForm.getCcExpiryMonth(), customerForm.getCcExpiryYear())) {
			throw new ApiException.ValidationFailure("Please enter valid expiration date");
		}
	}
	...
	private boolean expiryDateIsInvalid(String ccExpiryMonth, String ccExpiryYear) {
		...
		return currentYearMonth.isAfter(YearMonth.of(providedYear, providedMonth));
	...
	private long performPlaceOrderTransaction(
		...
		try {
			...
			long customerId = customerDao.create(
					...
					ccNumber, new java.sql.Date(date.getTime()));
