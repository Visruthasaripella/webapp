...
public class ApiResource {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Category findCategoryByName(@PathParam("category-name") String categoryName,
                                          @Context HttpServletRequest httpRequest) {
        try {
    ...
    public List<Book> suggestedBooksByCategoryName(@PathParam("category-name") String categoryName,
                                          ...
                                          @Context HttpServletRequest httpRequest) {
        try {
            List<Book> books = bookDao.findRandomBooksByCategoryName(categoryName, limit);
            if (books == null) {
                throw new ApiException(String.format("No such category name: %s", categoryName));
            }
            return books;
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public List<Book> getTopRatedBooks(@QueryParam("limit") @DefaultValue("6") int limit,
                                                   @Context HttpServletRequest httpRequest) {
        try {
            List<Book> books = bookDao.getTopRatedBooks(limit);
            if (books == null) {
                throw new ApiException("Failed to fetch top rated books");
            }
            return books;
        } catch (Exception e) {
            throw new ApiException("Failed to fetch top rated books", e);
        }
    }
    @POST
    @Path("orders")
